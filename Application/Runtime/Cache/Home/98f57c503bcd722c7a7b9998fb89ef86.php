	...
	<div class="sidebar-nav">
		...
		<ul id="Input-menu" class="nav nav-list collapse in">
			...
			<?php if($typeUser['type'] == 0 or $typeUser['type'] == 2): ?><li><a href="/Home/Input/houseDataList">楼盘列表</a></li>
				...
				<div class="span10">
					<h1 class="page-title" style="font-family:Helvetica Neue, Helvetica, Arial, sans-serif">用户列表</h1>
					<div class="well">
						<table class="table">
							 <thead>
								<tr>
									...
									<th class="head1">客户详细资料</th>
									<th class="head1">客户最高风险承受指数</th>
									<th class="head1">最低预期收益指数</th>
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="gradeX">
										...
										<td>
											<a href="<?php echo U('customerDataIndex?id='.$vo['id']);?>" class="btn btn-primary btn-rounded"><i class="iconfa-pencil"></i> 编辑</a>
										</td>
			...
			<input type='hidden' id='hid_url' value='/Home/Input/customerDataList'>
			<footer>
	<hr>
